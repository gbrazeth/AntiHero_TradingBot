...
*   **Contenção do Filtro MIN_NOTIONAL:** Na Binance, as fatias de Take Profit precisam valer no mínimo 5 USDT para serem aceitas. Desenvolvemos uma trava matemática que analisa o tamanho do alvo; se ficar abaixo do limite da corretora, o bot automaticamente agrupa as ordens de saída para ultrapassar 6 USDT, blindando as suas operações contra rejeições silenciosas (`-1013 Filter Failure`).
*   **Radar Autônomo de Estado (Polling de 15s):** As ordens alvo (Limit) costumavam bater na corretora e o robô não ficava sabendo de imediato. Para resolver o "ponto cego", escrevemos do zero um motor de rastreamento no backend. A cada 15 segundos, o cérebro faz um "ping" direto nos saldos da Binance. Se ele flagrar que uma parcial foi atingida nativamente, ele assume o controle no mesmo segundo e puxa automaticamente o Stop para o zero a zero (Break-Even), notificando você pelo Telegram.
